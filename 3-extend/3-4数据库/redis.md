...
## 缓存雪崩、缓存击穿、缓存穿透
缓存雪崩：同一时间类，大量缓存失效，导致数据请求打到数据库，导致数据库崩溃。
解决：设置不同的过期时间；
缓存击穿：一个热点缓存过期，同一时间大量读取的请求打到数据库导致数据库崩溃。
解决：如果缓存不存在数据加锁，包装只有一个线程请求数据库。
缓存穿透：请求一个不存在的数据，直接打到数据库导致数据库崩溃。
解决：布隆过滤器或不存在的数据添加特殊标记

## 缓存一致性
描述：修改、删除源数据时，需要调整已有缓存包装缓存数据和源数据的一致。
### （错误）1、无论源数据是更新还是删除，对于缓存都采取删除操作原因分析
如果是update缓存，需要考虑并发问题，会使得解决方案更复杂，并且缓存本身的更新频率不应该高；
直接采用删除策略，读取缓存的时候如果没有，去加载最新的源数据。
### （错误）2、先删除缓存的一致性问题（以mysql为例）
（1）先删除缓存，再更新mysql：高并发时，会导致其它线程可能读取到旧的mysql数据（更新mysql之前读取），
导致删除缓存操作无效。
### 3、先更新mysql再删除缓存
（1）如果对一致性要求较高，需要引入删除缓存的重试机制，避免删除失败；
（2）如果缓存删除失败对业务有影响，需要包装缓存删除和mysql更新在同一个事务，缓存删除失败抛出异常，mysql回滚；
（3）兜底策略，读取mysql的binlog，去异步删除缓存。
### 极端问题：
（1）mysql读写分离，主从同步需要时间；mysql更新后删除缓存，高并发情况下，还是可能读取到从库的旧数据。
解决方案：mysql更新后延迟一段时间再去删除redis缓存，读取mysql从库的binlog兜底删除缓存。
（2）
